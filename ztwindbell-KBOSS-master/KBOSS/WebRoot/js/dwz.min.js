	...
	$.fn.extend({
		...
		layoutH: function($refBox){
			return this.each(function(){
					...
					if(iLayoutH){//20150826张涛兼容性调整增加此判断条件
		...
		return this.each(function(){
			...
			$box.find("li>a").click(function(){
				...
				$.post($a.attr("href"), {}, function(html){
					...
					//navTab.closeAllTab(); 2015.11.11取消，切换主菜单，不清除选项卡
				});
				return false;
			});
		});
...
function navTabAjaxDone(json){
	...
	if (json[DWZ.keys.statusCode] == DWZ.statusCode.ok){
		...
		if ("closeCurrent" == json.callbackType) {
			if(json.rel&&json.navTabId) {
				//20151119 因后台刷新有问题，先关闭当前navtab，再行刷新
				setTimeout(function(){
					navTab.closeCurrentTab(json.navTabId);
					var $pagerForm = $("#pagerForm", navTab.getPanel(json.navTabId));
					var args = $pagerForm.size()>0 ? $pagerForm.serializeArray() : {}
					navTabPageBreak2(args, json.rel,json.navTabId);
				}, 100);
			}else {
...
//20151119张涛，为刷新指定navtab的指定区域而建
function dwzPageBreak2(options,navTabId){
	var op = $.extend({ targetType:"navTab", rel:"", data:{pageNum:"", numPerPage:"", orderField:"", orderDirection:""}, callback:null}, options);
	var $parent = op.targetType == "dialog" ? $.pdialog.getCurrent() : navTab.getPanel(navTabId);
...
function navTabPageBreak2(args, rel,tabid){
	dwzPageBreak2({targetType:"navTab", rel:rel, data:args},tabid);
}
/**
 * 处理navTab中的分页和排序
